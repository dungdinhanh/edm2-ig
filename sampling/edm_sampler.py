...
def edm_sampler_compress(cond_net: Any,
                latents: torch.Tensor,
                uncond_net: Optional[Any] = None,
                class_labels: Optional[torch.Tensor] = None,
                G: float = 1.0,
                guidance_list: Optional[list] = None,
                compression_rate: Optional[list] = None,
                num_steps: int = 32,
                sigma_min: float = 0.002,
                sigma_max: float = 80.0,
                rho: float = 7.0,
    ...
    """Second order EDM sampler."""
    # Determine denoising function.
    if guidance_list is None:  # Regular sampling mode. If uncond. net is available, then guidance is used.
        if uncond_net is None:  # No guidance.
            denoise_fn = lambda *args: cond_net(*args).to(torch.float64)
        else:  # Apply guidance.
            denoise_fn = lambda *args: uncond_net(*args).to(torch.float64).lerp(cond_net(*args).to(torch.float64), G)
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        # Update denoising function on the fly when using guidance interval.
        if guidance_list is not None:
            if guidance_list[i] > 0:
                if verbose:
                    print(f'Using guidance (G = {G})...')
                denoise_fn = lambda *args: uncond_net(*args).to(torch.float64).lerp(cond_net(*args).to(torch.float64), G * compression_rate[i])
            else:
                if verbose:
                    print('No guidance...')
                denoise_fn = lambda *args: cond_net(*args).to(torch.float64)
