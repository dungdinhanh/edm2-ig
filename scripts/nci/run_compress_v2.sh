...
#PBS -o output_nci/compress_guidance_edm2_k1_v2.txt
...
cmd="WORLD_SIZE=1 RANK=0 MASTER_IP=127.0.0.1 MASTER_PORT=29510 MARSV2_WHOLE_LIFE_STATE=0 python generate_compress_dist.py \
 ...
 --zip_dir IntG/compress_${skip} --skip ${skip} --guidance_interval '[17, 22]' --guidance_scale ${scale} --save-num 768 --fix_seed --base_folder ${base_folder}"
echo ${cmd}
# eval ${cmd}
done
done
